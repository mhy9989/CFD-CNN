...
def reserve_schedule_sampling_exp(itr, batch_size, args):
    ...
    r_random_flip = np.random.random_sample(
        (batch_size, args.data_previous - 1))
    r_true_token = (r_random_flip < r_eta)

    random_flip = np.random.random_sample(
        (batch_size, args.data_after - 1))
    ...
    for i in range(batch_size):
        for j in range(args.total_length - 2):
            if j < args.data_previous - 1:
                if r_true_token[i, j]:
                    real_input_flag.append(ones)
                else:
                    real_input_flag.append(zeros)
            else:
                if true_token[i, j - (args.data_previous - 1)]:
                    real_input_flag.append(ones)
                else:
                    real_input_flag.append(zeros)
...
def schedule_sampling(eta, itr, batch_size, args):
    ...
    real_input_flag = []
    for i in range(batch_size):
        for j in range(args.data_after - 1):
            if true_token[i, j]:
                real_input_flag.append(ones)
            else:
                real_input_flag.append(zeros)
